<script lang="ts">
    ...
    let {
        rackData = [],
        totalRacks = 0,
    }: { rackData: RackRow[]; totalRacks: number } = $props();

    // Cast once so templates don't need `as any`
    const table = new TableHandler<RackRow>(rackData, {
        rowsPerPage: 10,
        totalRows: totalRacks,
    });
    ...
    function doExport() {
        window.location.href = routeUrl(exportMethod());
    }
    function doTemplate() {
        window.location.href = routeUrl(template());
    }
    ...
    // Success indicator shown briefly when import completes
    let importSuccess = $state<string | null>(null);
    // timer id so repeated imports reset the timeout
    ...
    async function onFileSelected(e: Event) {
        ...
        try {
            const res = await fetch(routeUrl(importMethod()), {
                method: 'POST',
                body: form,
            });
            if (!res.ok) {
                ...
                // indicate success: invalidate table and show a green ring + success message
                table.invalidate();
...
<Layout>
    <div class="mx-auto max-w-8xl p-4 space-y-4">
        ...
        {#if importError}
            <div
                class="rounded-md bg-destructive/10 text-destructive text-sm px-4 py-2 flex items-center justify-between"
            >
        ...
        <!-- Import success (temporary) -->
        {#if importSuccess}
            <div
                class="rounded-md bg-green-50 text-green-800 text-sm px-4 py-2 flex items-center justify-between"
                role="status"
                aria-live="polite"
            >
                <span>{importSuccess}</span>
                <button
                    onclick={() => (importSuccess = null)}
                    class="ml-4 font-bold">✕</button
                >
            </div>
        {/if}
        ...
        <Card.Root
            class={importSuccess
                ? 'relative ring-2 ring-green-400'
                : 'relative'}
        >
            ...
            <Card.Content class="p-0">
                <Datatable table={t}>
                        ...
                        <Table.Header>
                            ...
                            <Table.Row class="hover:bg-transparent">
                                ...
                                <Table.Head class="w-28">
                                    <ThSort table={t} field="rack_no"
                                        >Rack No</ThSort
                                    >
                                </Table.Head>
                                <Table.Head class="w-32">
                                    <ThSort table={t} field="item_code"
                                        >Item Code</ThSort
                                    >
                                </Table.Head>
                                <Table.Head>
                                    <ThSort table={t} field="part_name"
                                        >Part Name</ThSort
                                    >
                                </Table.Head>
                                <Table.Head class="w-20">
                                    <ThSort table={t} field="part_hydrolis"
                                        >Hydrolis</ThSort
                                    >
                                </Table.Head>
                                <Table.Head class="w-32">
                                    <ThSort table={t} field="supplier"
                                        >Supplier</ThSort
                                    >
                                </Table.Head>
                                <Table.Head class="w-28">
                                    <ThSort table={t} field="type_tractor"
                                        >Tractor</ThSort
                                    >
                                </Table.Head>
                                <Table.Head class="w-20">Sample</Table.Head>
                                <Table.Head class="w-24 pr-4 text-right"
                                    >Actions</Table.Head
                                >
                            </Table.Row>
                        </Table.Header>
                        <Table.Body>
                            {#each table.rows as row (row.id)}
                                <Table.Row class="group text-sm">
                                    <Table.Cell
                                        class="pl-4 font-mono text-xs text-muted-foreground"
                                        >{row.id}</Table.Cell
                                    >
                                    <Table.Cell class="font-medium"
                                        >{row.rack_no ?? '—'}</Table.Cell
                                    >
                                    ...
                                    <Table.Cell
                                        class="text-xs text-muted-foreground"
                                        >{row.supplier ?? '—'}</Table.Cell
                                    >
                                    <Table.Cell
                                        class="text-xs text-muted-foreground"
                                        >{row.type_tractor ?? '—'}</Table.Cell
                                    >
                                    <Table.Cell
                                        class="text-xs text-muted-foreground"
                                        >{row.sample ?? '—'}</Table.Cell
                                    >
                    ...
                    {#snippet footer()}
                        <div
                            class="flex items-center w-full justify-between px-4 py-3 border-t text-sm text-muted-foreground"
                        >
                            <RowCount table={t} />
                            <Pagination table={t} />
                        </div>
