<script lang="ts">
    ...
    let listBasedOntype: ListBasedOnType = $state({
        kecil: [],
        besar: [],
        pallet: [],
    });
    ...
    $effect(() => {
        listBasedOntype = {
            ...
            pallet: list.filter(
                (item: listRack) => item.label_type === 'pallet',
            ),
        };
    });
    ...
    $effect(() => {
        ...
        if (listBasedOntype.kecil && listBasedOntype.kecil.length > 0) {
            ...
            kecilSheets.forEach((labels) => {
                ...
                const labelsWithId: LabelWithId[] = labels.map((label) => ({
                    ...label,
                    id: `label-${globalIndex++}`,
                }));
                sheets.push({
                    type: 'kecil',
                    labels: labelsWithId,
                    maxPerPage: smallLabel,
                });
            });
        }
        ...
        if (listBasedOntype.pallet && listBasedOntype.pallet.length > 0) {
            const palletSheets = chunkArray(
                listBasedOntype.pallet,
                palletLabel,
            );
...
<Navbar>
    <div class="space-y-6">
        <Card.Root>
            <Card.Header>
                <Card.Title class="text-center mt-4"
                    >Daftar Label untuk Dicetak</Card.Title
                >
                <Card.Description class="text-center mb-4">
                    Seret dan lepas untuk mengatur ulang urutan pencetakan
                    label.
                </Card.Description>
            </Card.Header>
            <Card.Content>
                <div class="w-full">
                    <div
                        class="flex gap-3 items-center justify-center my-4 flex-wrap"
                    >
                        <span class="text-sm text-gray-600"
                            >Total Sheets: {sheetCount}</span
                        >

                        <div class="flex gap-2 items-center border-l pl-3 ml-3">
                            <label for="pageType" class="text-sm font-medium"
                                >New Page Type:</label
                            >
                            <select
                                id="pageType"
                    ...
                    <div class="space-y-4">
                        <div
                            class="grid md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-4 2xl:grid-cols-6 gap-10"
                        >
                            {#each allSheets as sheet, pageIdx}
                                <div
                                    class="border-2 border-gray-300 p-3 bg-white rounded-lg shadow-md"
                                >
                                    <div
                                        class="mb-2 pb-2 border-b-2 border-gray-200 flex justify-between items-start"
                                    >
                                        <div class="flex-1">
                                            <h3
                                            ...
                                            >
                                                {sheet.labels
                                                    .length}/{sheet.maxPerPage} labels
                                            </p>
                                        </div>
                                        <button
                                    ...
                                    >
                                        {#each sheet.labels as label (label.id)}
                                            <div
                                                class="border-b last:border-0 p-2 hover:bg-gray-50 cursor-move transition-colors"
                                            >
                                                <p
        ...
        <Card.Root>
            <Card.Header>
                <Card.Title class="text-center mt-4 text-xl font-bold"
                    >Render Label</Card.Title
                >
                <Card.Description class="text-center mb-4">
